 ...
 *
 * Сервер про это знает: `turn-orchestrator.mjs` прямо называет финальный маркер
 * подсказкой, а не фактом, и закрывает ход только через окно стабилизации
 * (`turn-reconciler.mjs`, 3 с тишины). Здесь то же правило записано для
 * клиента — не вторым способом, а тем же самым, с тем же окном.
 *
...
/**
 * Окно стабилизации после финального маркера. Совпадает с
 * `THRESHOLDS.STABILIZATION_MS` в `server/turn-reconciler.mjs` намеренно:
 * разные окна на клиенте и сервере — это два разных ответа на вопрос
 * «ход закончился?», и расходились бы они молча.
 */
