...
export interface TurnCompletionOptions {
  ...
  onFailed: () => void;
  /**
   * Watchdog — не успешный финал. Он означает только, что браузер не получил
   * авторитетный вердикт вовремя. Старые вызовы без callback используют
   * обработчик failed, чтобы неопределённость никогда не превращалась в idle.
   */
...
export function awaitTurnCompletion(
  ...
  return new Promise<void>((resolve, reject) => {
    ...
    const timeoutId = setTimeout(() => {
      ...
      if (verdictPoller) clearInterval(verdictPoller);
      sessionFsm.clearIdleResolver(sidStr, requestGen);
      // Не оставляем последнюю `running`-проекцию как будто она всё ещё
      // подтверждена сервером. null здесь означает ровно «авторитетных данных
      // сейчас нет», а не «ход завершён».
