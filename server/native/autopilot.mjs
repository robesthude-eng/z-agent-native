...
export async function runFallbackPlan(plan, request, invoke, options = {}) {
  ...
  for (let index = 0; index < candidates.length; index++) {
    ...
    // Модель не знала, какая она модель: в системном промпте имени не
    // было, и на вопрос «какая ты модель» LLM отвечала догадкой из
    // претрейна — обычно чужим брендом. Здесь известен реальный кандидат
    // этой попытки, так что при переключении на резервную модель строка
    // останётся верной.
    const identity = `Runtime model identity: this reply is generated by "${modelKey(candidate)}" (provider id "${candidate?.providerID || ''}", model id "${candidate?.modelID || ''}"). If the user asks which model or provider is answering, state exactly this pair and never guess another vendor, brand or version.`;
