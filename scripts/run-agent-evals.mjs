...
const manifest = JSON.parse(
  fs.readFileSync(path.join(root, "evals/coding-agent.json"), "utf8"),
);
...
function parseModel(value) {
  ...
  if (slash <= 0 || slash === text.length - 1)
    throw new Error("Pass --model provider/model (or Z_AGENT_EVAL_MODEL)");
...
function treeHash(rootDir) {
  ...
  const walk = (dir) => {
    for (const entry of fs
      ...
      if (COPY_IGNORES.has(entry.name) || entry.name === ".agent-home")
        continue;
...
function commandCheck(command, cwd) {
  ...
  try {
    ...
    return {
      configured: true,
      ok: true,
      output: String(output || "").slice(-3000),
    };
  } catch (err) {
...
function scoreCase(
  item,
  { text, tools, beforeHash, afterHash, workspace, outcome },
) {
  const lower = text.toLowerCase();
  const pathHits = item.mustMentionPaths.map((value) =>
    lower.includes(String(value).toLowerCase()),
  );
  const anyHits = item.mustMentionAny.map((value) =>
    lower.includes(String(value).toLowerCase()),
  );
  const fileChecks = (item.expectFiles || []).map((expected) => {
    ...
    return {
      ...
      ok:
        exists &&
        (expected.contains == null || content.includes(expected.contains)),
  ...
  const requiresMutation =
    !item.forbidMutation && String(item.agent || "") === "implement";
  const mutationOk = item.forbidMutation
    ? !mutated
    : requiresMutation
      ? mutated
      : true;
  const toolErrors = tools.filter((tool) => tool.isError).length;
  const delegated = tools.some(
    (tool) =>
      tool.tool === "task" &&
      String(tool.input?.agent || "").toLowerCase() ===
        String(item.agent || "").toLowerCase(),
  );

  const criteria = [
    {
      name: "final_mentions_paths",
      weight: 25,
      ok: pathHits.every(Boolean),
      detail: `${pathHits.filter(Boolean).length}/${pathHits.length}`,
    },
    {
      name: "final_mentions_signal",
      weight: 15,
      ok: anyHits.some(Boolean),
      detail: `${anyHits.filter(Boolean).length}/${anyHits.length}`,
    },
    {
      name: "turn_completed",
      weight: 15,
      ok: String(outcome || "") === "completed",
      detail: String(outcome || ""),
    },
    {
      name: "specialized_worker",
      weight: 10,
      ok: item.ciSmoke ? true : delegated,
      detail: item.ciSmoke
        ? "fixture smoke"
        : delegated
          ? item.agent
    ...
    },
    {
      ...
      weight: 10,
      ok: toolErrors === 0,
      detail: String(toolErrors),
    },
    {
      name: "mutation_policy",
      weight: 10,
      ok: mutationOk,
      detail: mutated ? "mutated" : "unchanged",
    },
    {
      name: "expected_files",
      weight: 10,
      ok: fileChecks.length === 0 || fileChecks.every((check) => check.ok),
      detail:
        fileChecks
          .map((check) => `${check.path}:${check.ok ? "ok" : "fail"}`)
          .join(",") || "n/a",
    },
    {
  ...
  const applicable = criteria.filter((criterion) => {
    if (criterion.name === "expected_files" && fileChecks.length === 0)
      return false;
    if (criterion.name === "external_verification" && !verify.configured)
      return false;
    return true;
  });
...
const model = parseModel(
  smoke
    ? "fixture/coding-e2e"
    : cli.model ||
        process.env.Z_AGENT_EVAL_MODEL ||
        process.env.Z_AGENT_DEFAULT_MODEL,
);
...
process.env.Z_AGENT_TELEMETRY_FILE = path.join(
  tempRoot,
...
if (!smoke) {
  ...
  if (!baseURL || !key)
    throw new Error(
      "Real evals need --base-url and --api-key (or Z_AGENT_EVAL_BASE_URL/Z_AGENT_EVAL_API_KEY) for the isolated eval runtime.",
    );
  providerConfigs.upsertProviderConfig(ownerId, {
    id: model.providerID,
    name: `Eval ${model.providerID}`,
    protocol,
    baseURL,
    enabled: true,
  });
...
let cases = manifest.cases.filter((item) =>
  smoke ? item.ciSmoke : !item.ciSmoke,
);
...
for (const [index, item] of cases.entries()) {
  ...
  try {
    message = await runTurn({
      ...
      system: item.ciSmoke
        ? ""
        : `Evaluation constraint: use task(agent="${item.agent}") as the primary specialized worker for this task. Base the final answer on repository evidence and cite concrete relative paths.`,
    });
  } catch (err) {
    thrown = err;
    message =
  ...
  const scoring = scoreCase(item, {
    text,
    tools,
    beforeHash,
    afterHash,
    workspace,
    outcome: message?.info?.outcome?.status || (thrown ? "failed" : ""),
  });
  const result = {
    id: item.id,
    agent: item.agent,
  ...
  results.push(result);
  console.log(
    `[${index + 1}/${cases.length}] ${item.id}: ${result.score} ${result.error ? `ERROR ${result.error}` : ""}`.trim(),
  );
...
if (cli.baseline) {
  ...
  const priorById = new Map(
    (baseline.results || []).map((item) => [item.id, item]),
  );
  ...
  report.comparison = {
    ...
    averageDelta: compared.length
      ? Number(
          (
            compared.reduce((sum, item) => sum + item.delta, 0) /
            compared.length
          ).toFixed(2),
        )
      : null,
    improved: compared.filter((item) => item.delta > tolerance).length,
    unchanged: compared.filter((item) => Math.abs(item.delta) <= tolerance)
      .length,
    regressed: regressions.length,
    cases: compared,
  };
  console.log(
    `Baseline comparison: ${compared.length} matched, ${report.comparison.improved} improved, ${report.comparison.regressed} regressed beyond ${tolerance} points, avg delta ${report.comparison.averageDelta ?? "n/a"}`,
  );
...
const output = path.resolve(
  cli.output ||
    path.join(
      root,
      "evals",
      smoke ? "latest-smoke-report.json" : "latest-report.json",
    ),
);
fs.writeFileSync(output, JSON.stringify(report, null, 2));
console.log(
  `Eval score: ${report.averageScore}/100; pass>=80: ${report.passed}/${report.caseCount}`,
);
