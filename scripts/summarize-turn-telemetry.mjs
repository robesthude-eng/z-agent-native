...
const file = path.resolve(
  argValue("--file") ||
    process.env.Z_AGENT_TELEMETRY_FILE ||
    path.join(dataDir, "turn-telemetry.jsonl"),
);
const last = Math.max(
  1,
  Math.min(100_000, Number(argValue("--last")) || 1_000),
);
...
const rows = fs
  ...
  .map((line, index) => {
    try {
      ...
      throw new Error(`Invalid telemetry JSONL near selected row ${index + 1}`);
    }
  });
...
const summary = {
  ...
  averages: {
    ...
    inputTokens: average(
      rows.reduce((n, row) => n + Number(row?.tokens?.input || 0), 0),
    ),
    outputTokens: average(
      rows.reduce((n, row) => n + Number(row?.tokens?.output || 0), 0),
    ),
  },
  ...
  estimatedCostUsd:
    Math.round(
      rows.reduce((n, row) => n + Number(row?.estimatedCostUsd || 0), 0) * 1e8,
    ) / 1e8,
  models,
  tools: Object.fromEntries(
    Object.entries(tools).sort((a, b) => b[1].calls - a[1].calls),
  ),
};
